...
List<BbsDTO> bbsListTest = dao.bbsListTest(param, page_no);
dao.close();
...
%>
	...
	<table style="right:clear;">
		...
		<tr>
			<td><%=bbsListTest.size()-i %></td>
			<%if(login){%>
			<td><a href="view.jsp?idx=<%= bbsListTest.get(i).getIdx()%>&bordernum=<%=bbsListTest.size()-i %>"><%= bbsListTest.get(i).getTitle() %></a></td>
			<% }else if(!login){ %>
	...
	<table>
		<tr>
			<td style="text-align:center;">
				<% if(startnum>1){ %>
				<a href="list.jsp?page_no=<%=(startnum-2)*10+1 %>&startnum=<%=startnum-1%>"><<</a>
				<%} %> 
				<% int endnum = (total_count/10)+1<startnum*10?((total_count-1)/10)+1:startnum*10;
				for(int i = 1+((startnum-1)*10) ; i<=endnum; i++ ){%>
				<a href="list.jsp?page_no=<%=i %>&startnum=<%=startnum%>"><%=i %></a> |
				<%} %>
				<% if((total_count/10)+1>startnum*10){ %>
				<a href="list.jsp?page_no=<%=startnum*10+1 %>&startnum=<%=startnum+1%>">>></a>
				<% }%>
